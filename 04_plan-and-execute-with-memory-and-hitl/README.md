...
plan_agent/
  ...
  main.py        # CLI
tests/           # 54 офлайн-тести (LLM, мережа та Chroma замокані)
```
...
модель `deepseek/deepseek-v4-flash`, embeddings `qwen/qwen3-embedding-8b`. Усе — константи в
`config.py`. Structured outputs через `with_structured_output(..., method="function_calling")`.

**Чому не `xiaomi/mimo-v2.5`** (модель за замовчуванням у цьому репозиторії): на ній прогін
демо тонув у `429 temporarily rate-limited upstream` — повні прогони тривали десятки хвилин
або не доходили до кінця. `deepseek/deepseek-v4-flash` віддає той самий tool calling і
structured outputs, і повний `./run.sh` вкладається у ~6.5 хвилини без жодного 429. Змінюється
одним рядком у `config.py`.

**Тайм-аути обов'язкові.** У `ChatOpenAI` тайм-аута за замовчуванням немає: коли провайдер
приймає з'єднання й не відповідає, httpx чекає у `socket.recv` вічно, і агент німо висне
посеред кроку. Тому `LLM_TIMEOUT = 90 с` і `max_retries = 2` і для чату, і для embeddings;
зависання стає звичайною помилкою, яку демо ловлять і повторюють.
...
source .venv/bin/activate && pytest    # 54 тести, офлайн, без витрат на API
```
...
Прогін `./run.sh` від 2026-07-28, модель `deepseek/deepseek-v4-flash`, `max_iterations=8`.
Повний прогін (10 запусків агента, ~100 послідовних викликів LLM) — **6 хв 39 с**, жодного 429.

| Демо | Кроків плану | Час | Рішення replanner-а | Результат |
|---|---|---|---|---|
| `plan_simple` — 200 EUR + безвіз | 2/2 | 27.8 с | continue, finish | ✅ `currency` + `search_knowledge` |
| `plan_complex` — 3 ночі у Кракові | 3/4 | 59.1 с | continue×2, **finish** | ✅ geocode → weather → RAG, 4-й крок визнано зайвим |
| `rag_reference` — скасування + страховка | 2/2 | 44.6 с | continue, finish | ✅ лише `search_knowledge`, живих API не торкнувся |
| `rag_live` — погода Львова + 100 USD | 3/3 | 30.3 с | continue×2, finish | ✅ geocode → weather → `currency(100, USD)`, RAG **не** викликано |
| `hitl` approve | 3/3 | 24.2 с | continue×2, finish | ✅ interrupt → APPROVE → `WE-КРА-20260810` |
| `hitl` reject | 4/4 | 93.5 с | continue×3, finish | ✅ interrupt → REJECT → `book_hotel` НЕ виконано |
| `hitl` edit | 3/3 | 136.7 с | continue×2, finish | ✅ виконано з аргументами людини: `WE-КРА-20260811`, 1 ніч, 95 EUR |
| `persistence` | 1/4 → 4/4 | 22.4 с | continue×3, finish | ✅ «падіння» після кроку 1, новий процес довів до кінця |
| `threads` | 2/2 | 17.0 с | continue, finish | ✅ свіжий thread порожній, `persist-001` не змінився |

**Plan-and-Execute працює за задумом.** Planner тримає розумну гранулярність (2–4 кроки) і не
планує того, чого не просили — у жодному з не-бронювальних кейсів `book_hotel` не з'явився ні
в плані, ні у викликах. Ланцюжок `geocode → weather` не порушено ані разу, хоча правило живе
лише в системному промпті.

**Replanner не декорація.** У `plan_complex` план мав 4 кроки, але після третього replanner
відповів `finish`: погода, ціни й туристичний збір уже були зібрані, а четвертий крок
(«сформувати відповідь») нічого не додавав — цю роботу однаково робить вузол `respond`. Тобто
рішення про завершення ухвалюється за фактичними результатами, а не за довжиною плану.
...
`geocode`/`weather`/`currency` і **жодного** RAG. Маршрутизацію робить модель за
docstring-ами, а не наш код. У сценаріях бронювання агент сам починав із бази знань (правила
скасування, ціни, віза), і лише потім переходив до дії.

**Persistence.** Процес 1 пройшов `planner → executor → approval → act` і обірвався: в
`agent_state.db` лишився крок 1/4 і `next=('replanner',)`. Процес 2 — інший інтерпретатор,
...
`persist-001` (до і після — крок 4, та сама ціль).

**HITL.** Усі три сценарії відпрацювали на одному й тому самому графі, різниця лише у
`Command(resume=...)`:

```
approval │ ⚠ book_hotel({'hotel_name': 'Rynek Inn', …}) — чекаю рішення людини (interrupt)
approval │ APPROVE                          → act │ book_hotel → {'status': 'booked', …}
approval │ REJECT — занадто дорого          → act │ book_hotel: НЕ виконано (відхилено людиною)
approval │ APPROVE (аргументи змінено людиною) → act │ book_hotel → {'reference': 'WE-КРА-20260811', 'nights': 1, 'total_cost_eur': 95.0}
```

На `reject` агент не впав і не спробував обійти відмову: крок отримав результат «ВІДХИЛЕНО»,
replanner пішов далі, фінальна відповідь чесно каже, що бронювання немає. У `edit` виконалися
саме аргументи людини, а не ті, що просила модель.

**Що знадобилося виправити після живих прогонів** (усе — у промптах, не в графі):
- Модель відповідала на крок «забронювати» текстовою табличкою з «потрібне підтвердження
  користувача» і **не викликала** `book_hotel` — тобто сама імітувала HITL і `interrupt()` не
  спрацьовував. Правило 4 системного промпту тепер прямо каже: шлюз реалізує граф, модель має
  викликати інструмент, а не питати дозволу текстом.
- `currency(1, USD)` замість `currency(100, USD)` на запит про 100 доларів — додано правило
  переносити числа з запиту в аргументи без змін.
- Planner нумерував кроки в самому тексті (`1. 1. geocode(...)`) — попросили не нумерувати.

**Що лишилося кривим у моделі.**
- Текст кроку плану досі часто є сигнатурою інструменту з конкретними параметрами
  (`book_hotel(name="Rynek Inn", check_in="2026-08-10", nights=2, …)`). У сценарії `edit` це
  видно в артефакті: опис кроку каже «2 ночі за 180 EUR», а виконано 1 ніч за 95 EUR, бо
  аргументи підмінила людина. Джерело правди — результат кроку, не його опис.
- Назву міста нормалізує непослідовно (`Krakow` / `Kraków` / `Краків`), і це протікає в номер
  бронювання (`WE-KRA-…` проти `WE-КРА-…`).
- У сценарії `reject` планувальник додав зайвий `geocode` перед бронюванням — плани іноді
  довші за потрібне (4 кроки замість 3).

**Замір, куди йде час.** Один запит — це 2 + 2×N послідовних викликів LLM (planner, executor і
replanner на кожен крок, respond у кінці), тому 4-кроковий план = ~10 викликів, які не
паралелізуються за визначенням. Медіана виклику 2–6 с, `respond` 3–14 с, embedding-запит для
`search_knowledge` 0.3–8 с. Звідси 20–140 с на запит і ~6.5 хв на всі демо.

**Replanning гілкою `replan` у живих прогонах не пішов жодного разу** — модель обирала
`continue`, поки план мав сенс, а тоді `finish` (у `plan_complex` — раніше кінця плану). Це
очікувано: жоден інструмент не повернув помилки, яка вимагала б переписати хвіст. Сама гілка
відтворюється тестом `tests/test_graph.py::test_replan_replaces_only_remaining_steps`, де
перевіряється, що виконані кроки збереглися, а решта замінена.

**Запобіжник `max_iterations`** у живих прогонах теж не спрацював (плани короткі) —
перевіряється тестом `test_max_iterations_guard_stops_endless_replanning`.

## Тестування

54 офлайн-тести, жодного звернення до мережі, до платного API чи до справжньої ChromaDB:
