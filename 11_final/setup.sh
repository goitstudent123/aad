...
if [ ! -f .env ]; then
  # Шаблон лежить поруч як .env.example (сам .env у .gitignore, щоб ключ не поїхав у git).
  cat > .env <<'ENV'
OPEN_ROUTER_API_KEY=
...
LANGFUSE_INIT_PROJECT_ID=nexora-support
LANGFUSE_INIT_PROJECT_NAME=nexora-support
...
# Альтернатива Langfuse — LangSmith:
# LANGSMITH_TRACING=true
# LANGSMITH_API_KEY=
# LANGSMITH_PROJECT=hw3-mas-nexora
ENV
  echo "Створено .env — впиши туди OPEN_ROUTER_API_KEY."
fi

# Langfuse піднімається окремо: 6 контейнерів потрібні не всім.
