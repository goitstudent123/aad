...
from travel_agent.tools import CurrencyArgs, FactsArgs, GeocodeArgs, WeatherArgs
...
def test_facts_rejects_url():
    with pytest.raises(ValidationError) as exc:
        FactsArgs(title="https://uk.wikipedia.org/wiki/Львів")
    assert "посилання" in str(exc.value)
...
def test_geocode_parses_first_result(fake_get):
    fake_get({"results": [{"name": "Краків", "country": "Польща", "latitude": 50.06, "longitude": 19.94}]})
...
def test_facts_returns_extract(fake_get):
    fake_get(
        {
            "title": "Чернівці",
            "extract": "Місто на заході України.",
            "content_urls": {"desktop": {"page": "https://uk.wikipedia.org/wiki/Чернівці"}},
        }
    )
    assert tools.facts.invoke({"title": "Чернівці"})["extract"] == "Місто на заході України."
